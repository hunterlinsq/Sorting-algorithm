...
输出：输入序列的有序排列$(a'_1,a'_2,\cdots,a'_n)$,满足$a'_1\leq a'_2 \leq \cdots \le a'_n$

适用于：少量元素的排序。                
工作方式：类似于排序一手扑克牌。开始时，我们的左手为空并且桌上的牌面向下。然后。我们每次从桌上拿走一张牌并将它插入左手中的正确位置。为了找到一张牌的正确位置，我们从右到左将它与以在手中的每张牌进行比较，拿在左手上的牌总是排序好的，原来的这些牌是桌上牌堆中顶部的牌。
...
插入排序时间复杂度最好为O(n),最差为O(n^2),平均情况为O(n^2),空间复杂度为O(1)，是稳定的算法。
